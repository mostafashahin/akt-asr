...
do
    ...
    for task in $tasks
    do
        echo "asr_align: Child $childID: Start processing task $task" 
        ...
        if [ $align_stage -eq 5 ]; then
            ...
            python3 local/ctm_to_txtgrid.py -tg $LOCAL_OUT_DIR/txtgrids/primary_16b_$task.txtgrid -p -w -l $LOCAL_OUT_DIR/asr/data_$task/dict/au_dict -m asr2sampa.map $LOCAL_OUT_DIR/asr/data_$task/align/ $LOCAL_OUT_DIR/asr/data_$task/ $LOCAL_OUT_DIR/txtgrids/primary_16b_$task.wav  || { all_tasks=false && continue; }
            align_stage=6
            echo $align_stage > $LOCAL_OUT_DIR/asr/data_$task/align_stage
        fi
    done
