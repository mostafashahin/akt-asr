...
do
    ...
    log=$LOCAL_OUT_DIR/asr_align.log
    ...
    do
        ...
        if [ $align_stage -eq 4 ]; then
            local/align_to_phone_ctm.sh  $LOCAL_OUT_DIR/asr/data_$task $MDL 2>&1 | tee -a $log || { all_tasks=false && continue; }
            align_stage=5
        ...
        if [ $align_stage -eq 5 ]; then
            ...
            python3 local/ctm_to_txtgrid.py -tg $LOCAL_OUT_DIR/txtgrids/primary_16b_$task.txtgrid -p -w -l $LOCAL_OUT_DIR/asr/data_$task/dict/au_dict -m asr2sampa.map $LOCAL_OUT_DIR/asr/data_$task/align/ $LOCAL_OUT_DIR/asr/data_$task/ $LOCAL_OUT_DIR/txtgrids/primary_16b_$task.wav 2>&1 | tee -a $log || { all_tasks=false && continue; }
            align_stage=6
            echo $align_stage > $LOCAL_OUT_DIR/asr/data_$task/align_stage
        fi
    done
